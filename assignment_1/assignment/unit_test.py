...
def logistic_unit_test():
    ...
    assert testcase_check(train_x_norm1, testcase['train_x_norm1'], "normalize_per_pixel", True), "Fail test case"

    assert testcase_check(train_x_norm2, testcase['train_x_norm2'], "normalize_all_pixel", True), "Fail test case"
    print(train_x.shape)
    train_x = reshape2D(train_x)
    assert testcase_check(train_x, testcase['train_x2D'], "reshape2D", True), "Fail test case"

    train_x = add_one(train_x)
    assert testcase_check(train_x, testcase['train_x1'], "add_one", True), "Fail test case"
    ...
    for i in range(10):
        ...
        assert testcase_check(y_hat, test_dict['y_hat'], "feed_forward %d" % (i+1), True), "Fail test case"

        loss = classifier.compute_loss(train_y, y_hat)
        assert testcase_check(loss, test_dict['loss'], "compute_loss %d" % (i+1), True), "Fail test case"

        grad = classifier.get_grad(train_x, train_y, y_hat)
        assert testcase_check(grad, test_dict['grad'], "get_grad %d" % (i+1), True), "Fail test case"

        classifier.update_weight(grad, 0.001)
        assert testcase_check(classifier.w, test_dict['w_1'], "update_weight %d" % (i+1), True), "Fail test case"

        momentum = np.ones_like(test_dict['grad'])
        classifier.update_weight_momentum(grad, learning_rate, momentum, momentum_rate)
        assert testcase_check(classifier.w, test_dict['w_2'], "update_weight_momentum %d" % (i+1), True), "Fail test case"
...
def softmax_unit_test():
    ...
    assert testcase_check(train_x, testcase['train_x_norm'], "normalize", True), "Fail test case"

    train_y = create_one_hot(train_y)

    assert testcase_check(train_y, testcase['one_hot'], "create_one_hot", True), "Fail test case"
    ...
    for i in range(10):
        ...
        assert testcase_check(y_hat, test_dict['y_hat'], "feed_forward / softmax", True), "Fail test case"
        assert testcase_check(loss, test_dict['loss'], "compute_loss", True), "Fail test case"
        assert testcase_check(grad, test_dict['grad'], "get_grad", True), "Fail test case"
